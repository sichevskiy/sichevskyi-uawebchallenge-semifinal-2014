...
class WGAPI
{
    ...
    function globalwarTop($map_id = "globalmap", $order_by = "provinces_count")
    {
        $request_data = array();
        ...
        return $this->doRequest(sprintf($this->api_format_wot, $this->tld, "globalwar", "top"), $request_data);
    }

    function encyclopediaTanks()
    {
        $request_data = array();

        return $this->doRequest(sprintf($this->api_format_wot, $this->tld, "encyclopedia", "tanks"), $request_data);
    }

    function encyclopediaTankinfo($tank_id)
    {
        $request_data = array();

        $request_data['tank_id'] = $tank_id;

        return $this->doRequest(sprintf($this->api_format_wot, $this->tld, "encyclopedia", "tankinfo"), $request_data);
    }

    function tanksStats($account_id)
    {
        $request_data = array();

        $request_data['account_id'] = $account_id;

        return $this->doRequest(sprintf($this->api_format_wot, $this->tld, "tanks", "stats"), $request_data);
    }
